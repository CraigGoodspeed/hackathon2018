...
;
        ...
        private static readonly string getProjectsForContractor = "select p.id as id,p.name as projectName,p.description as projectDescription,date_created as dateCreated,com.description as communityName,worker.name as contractorName,s.description as Status,dbo.calcProjectPrice(f.id) as amount,[dbo].[calcPriorityForContractor] (p.ID) as priority from project p inner join category c on c.id = p.category_id left join funding f on f.id = p.funding_id inner join status s on s.id = p.status_id inner join community com on com.id= p.community_id left join contractor worker on worker.id = p.contractor_id order by priority desc;";
        ...
        public static List<Models.Project> ProjectsForContractor(Guid organisationId)
        {
            using (SqlConnection conn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["projectLinker.Properties.Settings.hackathon_devConnectionString"].ConnectionString))
            {
                SqlCommand cmd = conn.CreateCommand();
                cmd.CommandText = getProjectsForContractor;
                return getProjects(cmd);
            }
        }

        private static List<Models.Project> getProjects(SqlCommand cmd)
        {
